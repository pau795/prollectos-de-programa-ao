...
pygame.mixer.init()
soundUP = pygame.mixer.Sound('50.ogg')
soundLEFT = pygame.mixer.Sound('60.ogg')
soundDOWN = pygame.mixer.Sound('70.ogg')
soundRIGHT = pygame.mixer.Sound('80.ogg')
...
def note1(button):
	if button==1: return soundUP
	if button==2: return soundLEFT
	if button==3: return soundDOWN
	if button==4: return soundRIGHT
		...
		if turn and event.type == pygame.KEYDOWN:
			if not pressed:
				if event.key == pygame.K_UP:
					...
					note=note1(button)
					pressed=True
					if audio: soundUP.play()
				if event.key == pygame.K_LEFT:
					button=2
					change+=1
					note=note1(button)
					pressed=True	
					if audio: soundLEFT.play()
				if event.key == pygame.K_DOWN:
					button=3
					change+=1
					note=note1(button)
					pressed=True		
					if audio: soundDOWN.play()
				if event.key == pygame.K_RIGHT:
					button=4
					change+=1
					note=note1(button)
					pressed=True
					if audio: soundRIGHT.play()
			else: change+=1
			if not pressed and event.key == pygame.K_ESCAPE:
				estado=0
				sc=0
